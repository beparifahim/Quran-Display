...
export default function QuranDisplay({ surahData, allSurahs, onSurahChange }) {
  ...
  // The hook handles gapless transitions in the background
  const { activePlayer, audioRef0, audioRef1, getActiveAudio } = useGaplessAudio(surahData, null, () => {});

  // 60fps high-performance UI sync for exact word highlighting
  useEffect(() => {
    let animationFrameId;
    const syncUI = () => {
      const audio = getActiveAudio();
      if (isPlaying && audio) setCurrentTime(audio.currentTime);
  ...
  if (!surahData || !surahData.ayahs) return <div className="min-h-screen bg-black flex items-center justify-center text-white">Loading...</div>;
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-center items-center p-4 md:p-8 transition-colors duration-700 ${currentTheme.bg}`}>
      ...
      <audio ref={audioRef0} src={activePlayer === 0 ? surahData.audio_source : ''} crossOrigin="anonymous" />
      <audio ref={audioRef1} src={activePlayer === 1 ? surahData.audio_source : ''} crossOrigin="anonymous" />
          ...
          <div className="text-center w-full">
            <div className="flex flex-wrap justify-center gap-x-3 gap-y-6 md:gap-x-5" dir="rtl">
              {currentAyah.words.map((word, index) => {
                ...
                return (
                  <span key={index} className={`${fontClasses[fontSize].arabic} font-indopak transition-colors duration-75 ${isActiveWord ? 'text-green-400 drop-shadow-md' : currentTheme.text}`}>
                    {word.arabic}
                  </span>
                );
              })}
      ...
      {!isPlaying && (
        <div className="fixed inset-0 z-50 flex flex-col justify-between p-4 md:p-8 pointer-events-none transition-opacity duration-500">
          <div className="w-full flex justify-center pointer-events-auto">
            <button onClick={() => setShowNav(true)} className={`px-6 py-3 rounded-full border shadow-lg backdrop-blur-md flex items-center space-x-2 text-gray-300 hover:text-white ${currentTheme.menu}`}>
          ...
          <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-4">
            {allSurahs.map((surah) => (
              <div key={surah.surah_number} className="bg-gray-900 rounded-xl p-4 border border-gray-800">
                <button 
                  onClick={() => { onSurahChange(surah.surah_number); setShowNav(false); }}
